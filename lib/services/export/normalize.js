...
function prioritizeSkills(skills, priorities) {
  ...
  return [...skills].sort((left, right) => {
    const leftIndex = normalizedPriorities.findIndex((item) => left.toLowerCase().includes(item));
    const rightIndex = normalizedPriorities.findIndex((item) => right.toLowerCase().includes(item));
...
function estimatePageCount(sectionCount, bulletCount, totalCharacters, mode) {
  const densityDivisor = mode === "compact" ? 3600 : mode === "executive" ? 3000 : 3200;
  const weightedSize = totalCharacters + sectionCount * 180 + bulletCount * 90;
...
export function normalizeResumeForExport({ structuredResume, exportOptions, sessionContext = {} }) {
  ...
  for (const section of structuredResume.sections || []) {
    ...
    if (["skills", "technical-skills", "core-competencies"].includes(normalizedSection.id)) {
      const flattenedItems = (section.items || []).flatMap((item) => parseDelimitedSkills(item));
