...
export default function JobAnalysisPage() {
  ...
  return (
    <AppShell
      ...
      description="Break the target role into must-haves, preferred skills, responsibilities, seniority, and domain vocabulary before rewriting anything."
    >
      <SectionCard title="Analyze target role" eyebrow="Step 2">
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <label className="block" htmlFor="company-name">
            ...
            <input id="company-name" value={company} onChange={(event) => setCompany(event.target.value)} className="w-full rounded-2xl border border-[var(--line)] bg-white px-4 py-3 outline-none" />
          </label>
          <label className="block" htmlFor="role-title">
            <span className="mb-2 block text-sm font-semibold">Role</span>
            <input id="role-title" value={role} onChange={(event) => setRole(event.target.value)} className="w-full rounded-2xl border border-[var(--line)] bg-white px-4 py-3 outline-none" />
          </label>
        </div>
        ...
        <SectionCard title="Must-have skills" eyebrow="JD intelligence">
          <div className="mt-6 flex flex-wrap gap-3">
            {analysis.mustHaveSkills.map((item) => (
              <span key={item} className="rounded-full border border-[var(--line)] bg-white px-4 py-2 text-sm font-semibold">
                {item}
              </span>
            ))}
          </div>
        ...
        <SectionCard title="Role shape" eyebrow="Seniority">
          <div className="mt-6 space-y-3">
            <div className="rounded-2xl border border-[var(--line)] bg-white p-4">
              <p className="text-sm text-[var(--ink-soft)]">Seniority</p>
              <p className="mt-2 text-xl font-semibold">{analysis.seniority}</p>
            </div>
            <div className="rounded-2xl border border-[var(--line)] bg-white p-4">
              <p className="text-sm text-[var(--ink-soft)]">Domain fit</p>
              <p className="mt-2 text-xl font-semibold">{analysis.primaryDomain}</p>
            </div>
          </div>
        </SectionCard>
      </div>
        ...
        <SectionCard title="Requirements not supported by your resume" eyebrow="Gap analysis">
          <div className="mt-6 space-y-3">
            {(state.tailoringSession?.gaps?.length ? state.tailoringSession.gaps : analysis.gaps || []).map((gap) => (
              <div key={gap.name} className="rounded-2xl border border-[var(--line)] bg-white px-4 py-4">
                <div className="flex items-center justify-between gap-4">
                  <p className="font-semibold">{gap.name} not found</p>
                  <span className="rounded-full bg-[var(--surface-muted)] px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em]">
                    {gap.severity}
                  </span>
                </div>
